...
master	1	4核 6G	192.168.110.185 
k8s-master
node	1	4核 6G	
k8s-node1
node	1	4核 6G	
...
--fail-swap-on=false 去掉这个限制。修改 /etc/systemd/system/kubelet.service.d/10-kubeadm.conf，加入：
...
source  ~/.bash_profile
若加入集群口令忘记，重新生成:
[root@vmi175035 heapster]# kubeadm token create
bo7osg.2h8z3o2ohqbh7vf5
[root@vmi175035 heapster]# kubeadm token generate
a4vdjw.vfnsh0n46qjm747p
[root@vmi175035 heapster]# kubeadm token list
...
# 获取ca证书sha256编码hash值
[root@vmi175035 heapster]#  openssl x509 -pubkey -in /etc/kubernetes/pki/ca.crt | openssl rsa -pubin -outform der 2>/dev/null | openssl dgst -sha256 -hex | sed 's/^.* //'
...
[root@k8s-master ~]# kubectl drain app5-185 --delete-local-data --force --ignore-daemonsets
